...
def place_bet():
    ...
    with get_db_cursor() as (cursor, conn):
        ...
        cursor.execute("SELECT credits, username FROM users WHERE id = %s", (user_id,))
        user = cursor.fetchone()
...
def get_live_bets():
    ...
    with get_db_cursor() as (cursor, _):
        cursor.execute(
            """
            SELECT b.amount, b.direction, b.created_at, u.username
            FROM bets b
            JOIN users u ON b.user_id = u.id
            WHERE b.round_id = %s AND b.crypto = %s
